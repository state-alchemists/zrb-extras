...
ZrbExtras is a [Zrb](https://pypi.org/project/zrb/) project.
...
To start working with ZrbExtras, you need to have:
...
To get started, you need to activate ZrbExtras virtual environment and install a few packages (including Zrb). You can do this by invoking the following command in your terminal:
...
You can add a new application to ZrbExtras by invoking the following command:
...
You will be able to find the application under ZrbExtras's `src` directory.
...
To configure ZrbExtras, you need to create a `.env` file.
...
Once the environment file is updated, you can reload ZrbExtras by invoking `reload` in the terminal.
